...
func addCustomMultiLineBlock(pdf *gopdf.GoPdf, x, y, lineWidth, lineHeight float64, text string, alignCentre bool) (float64, float64) {
	//lines, _ := pdf.SplitText(title, 200)
	//titleWidth := availablePageWidth - logoSize - qrSize - 50
	lines := wrapTextLines(pdf, text, lineWidth-2*padding)
	for _, line := range lines {
		x, y = addCustomTextBlock(pdf, x, y, lineWidth, lineHeight, line, alignCentre)
	}
	return x, y
}
func addCustomMultiLineBlock2(pdf *gopdf.GoPdf, x, y, lineWidth, lineHeight float64, text []string, alignCentre bool) (float64, float64) {

	addCustomTextBlock(pdf, x, y, lineWidth, lineHeight, "", alignCentre)
	x, y = addCustomTextBlock2(pdf, x, y, lineWidth, lineHeight, text, alignCentre)

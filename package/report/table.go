...
func addTable(pdf *gopdf.GoPdf, x, y, rowHeight, padding float64, colWidth []float64, tableHeader []string, data [][]string, fontSize int) {
	...
	//fmt.Printf("Estmated number rows = %d =  pages  = %d\n", totalRows, totalPageNumber)
...
func addCustomTextBlock(pdf *gopdf.GoPdf, x, y, w, h float64, text string, alignCentre bool) (float64, float64) {
	...
	pdf.SetX(x)
	pdf.SetY(y)
	pdf.RectFromUpperLeftWithStyle(x, y, 136, h, "D")
	if alignCentre {
		pdf.CellWithOption(&rect, text, cellOptionCentre)
...
func addCustomTextBlock2(pdf *gopdf.GoPdf, x, y, w, h float64, text []string, alignCentre bool) (float64, float64) {
	y = y + h/2 -15
	for _, t := range text {
		val := 10.0
		rect := gopdf.Rect{
		...
		if alignCentre {
			pdf.CellWithOption(&rect, t, cellOptionCentre)
		} else {
			pdf.CellWithOption(&rect, t, cellOptionLeft)
		}
	}
...
func addTableleftAlign(pdf *gopdf.GoPdf, x, y, rowHeight, padding float64, colWidth []float64, tableHeader []string, data [][]string, fontSize int) {
	//calculate number of pages
	...
	addHrGreyH(pdf, y, 0.5)
	_, y = addFillRow(pdf, x, y, rowHeight, padding, colWidth, tableHeader, false)
	addHrGreyH(pdf, y, 0.5)
	...
	for _, d := range data {
		_, y = addRow(pdf, x, y, rowHeight, padding, colWidth, d, false)
		addHrGreyH(pdf, y, 0.5)
		//addVrGreyH(pdf, y, 0.5)
		if checkTableEndOfPage(pdf, y) {
			setFontBold(pdf, fontSize)
			_, y = addFillRow(pdf, leftMargin, topMargin, rowHeight, padding, colWidth, tableHeader, true)
			setFont(pdf, fontSize)
...
func addBlockab(pdf *gopdf.GoPdf, x, y, w, h float64, text string, alignCentre bool) (float64, float64) {
	pdf.RectFromUpperLeftWithStyle(x, y, w, h, "D")
